...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $stmt = $pdo->prepare("SELECT id FROM estudiantes WHERE documento_numero = ?");
        $stmt->execute([$documento_numero]);
        
        if ($stmt->rowCount() > 0) {
            throw new Exception('Ya existe un estudiante con este número de documento.');
        }

        // Insertar el nuevo estudiante
        $stmt = $pdo->prepare("
            INSERT INTO estudiantes (
                nombre, apellido, documento_tipo, documento_numero, 
...
<body>
   ...
   <script>
       ...
       document.querySelector('form').addEventListener('submit', function(e) {
           ...
           const documentoNumero = document.getElementById('documento_numero').value;

           // Validar teléfonos (solo números)
           if (!/^\d+$/.test(telefono) || !/^\d+$/.test(telefonoAcudiente)) {
               e.preventDefault();
               alert('Los números de teléfono deben contener solo números');
               return;
           }

           // Validar documento (solo números)
           if (!/^\d+$/.test(documentoNumero)) {
               e.preventDefault();
